    ...
    public class PackageList:ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync(Paging paging)
        {
            if (paging.InPageCount == 0)
            {
                ...
                return await Task.FromResult((IViewComponentResult)View("/Areas/Admin/Views/Package/Components/List.cshtml", _core.Deal.Get().OrderByDescending(o => o.Id).Skip(skip).Take(10)));
            }
            else
            {
                ...
                return await Task.FromResult((IViewComponentResult)View("/Areas/Admin/Views/Package/Components/List.cshtml", _core.Deal.Get().OrderByDescending(o => o.Id).Skip(skip).Take(paging.InPageCount)));
            }
        }
    }
}
